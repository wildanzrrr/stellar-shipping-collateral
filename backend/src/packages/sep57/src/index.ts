...
export type DataKey = {tag: "Initialized", values: void} | {tag: "Admin", values: void} | {tag: "AdminSigner", values: void} | {tag: "IdentityVerifier", values: void} | {tag: "Compliance", values: void} | {tag: "Name", values: void} | {tag: "Symbol", values: void} | {tag: "Decimals", values: void} | {tag: "Balance", values: readonly [string]} | {tag: "TotalSupply", values: void} | {tag: "UsedNonce", values: readonly [u64]};
...
export interface Client {
   ...
   */
  burn: ({from, amount, nonce, deadline, signature}: {from: string, amount: i128, nonce: u64, deadline: u32, signature: Buffer}, options?: MethodOptions) => Promise<AssembledTransaction<null>>

  /**
   * Construct and simulate a mint transaction. Returns an `AssembledTransaction` object which will have a `result` field containing the result of the simulation. If this transaction changes contract state, you will need to call `signAndSend()` on the returned object.
   */
  mint: ({to, amount, nonce, deadline, signature}: {to: string, amount: i128, nonce: u64, deadline: u32, signature: Buffer}, options?: MethodOptions) => Promise<AssembledTransaction<null>>
   ...
   */
  initialize: ({admin, identity_verifier, compliance, admin_signer, name, symbol, decimals}: {admin: string, identity_verifier: string, compliance: string, admin_signer: Buffer, name: string, symbol: string, decimals: u32}, options?: MethodOptions) => Promise<AssembledTransaction<null>>
...
export class Client extends ContractClient {
  ...
  constructor(public readonly options: ContractClientOptions) {
    super(
      new ContractSpec([ "AAAAAAAAAAAAAAAEYnVybgAAAAUAAAAAAAAABGZyb20AAAATAAAAAAAAAAZhbW91bnQAAAAAAAsAAAAAAAAABW5vbmNlAAAAAAAABgAAAAAAAAAIZGVhZGxpbmUAAAAEAAAAAAAAAAlzaWduYXR1cmUAAAAAAAPuAAAAQAAAAAA=",
